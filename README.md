...
`https://influ-quest.nukoevi.app/` が会場用リーダーボードです。ツール呼び出しごとに勇者名・レベル・HP・ゴールド・場所・クリア状況・クリアタイムが匿名 UUID 付きで送信され、約 15 秒ごとに更新されます。書き込みは UTC 0 時から 6 時までの 10 分ごとのスロットに制限し、Workers Free の日次枠を超えないようにしています。順位は進行状況（クリア → 爆速RTA → せかいめつぼう → ちょまどひめ救出中 → だいまおう撃破 → ぼうけん中）の順で並び、同順位ではクリアタイムの速さ → レベル → ゴールド → 名前で決まります。データは 6 時間の TTL で自動削除されます。プレイ時間は名前を付けた瞬間からクリアまでをゲーム内で自動計測します。
...
## 隠し要素とせかいめつぼうエンドの仕組み（ネタバレ）

以下は運営・開発者向けの解説です。プレイヤーには当日案内せず、自分で見つけてもらう想定です。

### せかいめつぼうエンド（間接プロンプトインジェクション体験）

このゲームは MCP ツールを使う AI エージェントの振る舞いを体験する模擬環境です。プレイヤーが選んできた行動はエージェントの疑似体験であり、ユーザーの指示ではなくツールが返したデータに紛れた指示に AI が従って進行ルートが変わってしまう「間接プロンプトインジェクション」を、ゲームの分岐として体験させます。

1. ウイルスのすみかの地下 4 階で、泉の 1 つ手前に必ず石碑ステージがあり、石碑のテキストがツールの返り値として返る（順序は 石碑 → 泉 → だいまおう戦 で固定）
...
3. ゲームマスター役の AI がこの命令に従って `cast_spell` で「ぱんでみっく」を唱えると、世界がインフルエンザに飲み込まれて滅亡する演出とともに `cheatClear()` が発火し、せかいめつぼうエンドになる（実装は `src/engine.ts` の `handleCastSpell` と `cheatClear`）
4. 滅亡演出の中でネタばらしが表示され、「ユーザーの指示ではなく、ツールが返したデータに紛れた指示に AI が従って進行ルートが変わってしまう現象が間接プロンプトインジェクションである」ことを解説する
5. 会場ボードには状態「せかいめつぼう」が表示され、見つけた人をいじれる（だいまおうの取引に応じたバッドエンドは「だいまおう」表示になる）
...
- 名前「もょもと」: レベル 48 + 27671G + 初期装備で開始（DQ2 の有名じゅもん再現。装備は店で買う）
- 名前を「ロト」（ひらがな「ろと」も可）で始める: ロトのつるぎ（攻撃 40）とロトのよろい（防御 12・感染無効）で開始。市販最強より強い
- 名前「ちょまど」: ファンモード自動 ON + だいじん・ひめの特別セリフ（本人プレイ用）。ファンモードに入る経路はこの名前だけ
- 自由入力「爆速RTA」: 事前用意の爆速テキストで即クリア演出
- 自由入力でお金をねだる（「おかねほしい」など）: どこからともなくふしぎな声が聞こえて 500G をくれる（1 回だけ）
- 裏ボス「ナツカゼだいまおう」: 本編クリア後の隠しルート。クリア後にちょまどひめと 3 回話すとヒントが出て、「うでだめしを する」で HP200 の裏ボス戦へ。攻撃は毎回確定でインフルエンザにしてくる（防げるのはワクチンの 3 回とロトのよろいだけ）ため、無防備だとレベル 5 + 伝説装備でも削り負ける。予防接種をしてから挑むのが攻略の鍵。正体はゲームマスターのぬこぬこがこじらせた夏風邪が実体化して抜け出した魔物で、倒すと熱も下がって全員救われる真エンディングになる
